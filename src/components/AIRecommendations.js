...
function AIRecommendations({ weather }) {
  ...
  if (condition === 'Rain' || condition === 'Drizzle') {
    suggestions.push('☂️ Carry an umbrella today.');
  }
  if (temp > 35) {
  ...
  if (condition === 'Snow') {
    suggestions.push('❄️ Snowy conditions — drive carefully and wear boots.');
  }
  if (condition === 'Thunderstorm') {
    suggestions.push('⛈️ Thunderstorm warning — stay indoors if possible.');
  }
